 ...
 * Lab Section:     002
 * Assignment Name: Lab 8 storing a sequence in a binary tree
 */
...
void split(Node *T, int r, Node **L, Node **R)
{
  ...
  if ( r <= rank_of_root){
    ...
    *L = T;
    split(T->right, r - rank_of_root - 1, &(*L)->right, R);
  }
  fix_size(T);
}
...
Node *insert_random(Node *T, int v, int r)
{
  ...
  // if random == 0 we insert at root, i.e. make a new root
  if (random == 0){  
  ...
  for (int i=1; i<n; i++) {
    ...
    if (did_x_beat_y(i, select(T,0)->key)) // can we insert at beginning?
      T = insert_random(T, i, 0);
	    ...
	    // W W W L L L W L
      int low_rank = 0, mid_rank, high_rank = T->size - 1;
      do{
        ...
        if (did_x_beat_y(i, select(T, mid_rank)->key))
          ...
          low_rank = mid_rank;
      } while (high_rank - low_rank != 1); // ends when a W and L are next to each other
      T = insert(T, i, high_rank);
    }
  }
  return T;
}
